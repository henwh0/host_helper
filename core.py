...
# import from local
from host_helper.config import EXIT_HOSTORY_FAIL, EXIT_SERF_ERROR, EXIT_INVALID_MODEL, EXIT_SLED_LOG_FAIL, Colors, cprint, VALID_MODELS
...
def run_sled_analysis(sledname, args, host_position=None, hostname=None, slots=None):
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        try:
            ...
            cprint(f"Hostory failed: {e}", Colors.RED)
        ...
        if postcode_future:
            try:
                ...
                cprint(f"Postcode collection failed: {e}", Colors.RED)
                raise SystemExit(EXIT_SLED_LOG_FAIL)
        else:
