...
def main() -> None:
    ...
    if args.errors:
        ...
        invalid_errors = [
            error for error in args.errors
            if error not in ERROR_PATTERNS
        ]
        if invalid_errors:
            parser.error(f"invalid error pattern(s): {', '.join(invalid_errors)}")
    ...
    cprint(f"Detected model : {model_name}", Colors.GREEN)
    cprint(f"Asset ID       : {asset_id}\n", Colors.GREEN)
