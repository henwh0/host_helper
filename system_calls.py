...
# imports from local
from host_helper.config import Colors, cprint, EXIT_SLED_LOG_FAIL, EXIT_SERF_ERROR, EXIT_HOSTORY_FAIL
...
def retrieve_sled_logs(sledname: str, host_position=None, slots=None):
    ...
    target_slots = slots if slots else ([host_position] if host_position is not None else [])
...
def run_hostory(sledname: str) -> None:
    ...
    result = subprocess.run(["hostory", "-sc", "--yard", sledname])
    if result.returncode != 0:
        cprint(f"hostory failed for {sledname}", Colors.RED)
        raise SystemExit(EXIT_HOSTORY_FAIL)
...
def resolve_target_information(target, args):
    ...
    if target.startswith("sled"):
        info = parse_serf_output(run_cmd(["serf", "get", f"name={target}", "--fields=model,asset_id"]))
        if not info:
            cprint(f"No info found for {target}.", Colors.RED)
            raise SystemExit(EXIT_SERF_ERROR)
        info = info[0]
        return target, info.get("model"), None, None, [], info.get("asset_id")

    # Target is a hostname — resolve to sled
    hostname = target
    fields = "parent_asset_tag,model,rack_sub_position_slot,asset_id"
    ...
    asset_id = host_info.get("asset_id")
    ...
    return sledname, sled_model_name, hostname, host_position, slots, asset_id